...
export class DataMixer<TMix extends DataMix = DataMix> {
  ...
  mix<T, TOptions extends unknown[]>(
    ...
    this.#seeps.set(infusion as DataInfusion<unknown, unknown[]>, seep as DataSeep<unknown, unknown[], TMix>);
...
class DataMix$Compound<TMix extends DataMix> implements DataCompound<TMix> {
  ...
  faucetFor<T, TOptions extends []>(infusion: DataInfusion<T, TOptions>, mix: TMix): DataFaucet<T> {
    ...
    let faucet = this.#faucets.get(infusion as DataInfusion<unknown, unknown[]>) as DataFaucet<T> | undefined;

    if (!faucet) {

      const seep = this.#seeps.get(infusion as DataInfusion<unknown, unknown[]>) as DataSeep<T, TOptions, TMix>;
...
function DataMix$createDefault<TMix extends DataMix>(
  ...
  return async (sink, supply) => await sinkValue(
      new DefaultDataMix(compound) as DataMix as TMix,
      sink,
      supply,
  );
}
